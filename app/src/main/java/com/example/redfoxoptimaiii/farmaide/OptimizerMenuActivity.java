...
//        }
        ...
        if(additives!=null && ingredients!=null) for (int i=0;i<additives.size();i+=1) ingredients.add(additives.get(i));
        ...
        try{
            ...
            for(int i=0;i<col-1;i+=1){
               if (i<8){
                   if (i%2==0) tableau[row-1][i] = -1*(nutri_req[i/2] - (nutri_req[i/2]*nutri_req_margin));
                   else tableau[row-1][i] = nutri_req[i/2] + (nutri_req[i/2]*nutri_req_margin);
               }
...
//            Toast.makeText(getApplicationContext(), matrix, Toast.LENGTH_LONG).show();
            ...
            if (simplex.isOptimized()) Toast.makeText(this, "Please add more ingredient", Toast.LENGTH_SHORT).show();
            else {
