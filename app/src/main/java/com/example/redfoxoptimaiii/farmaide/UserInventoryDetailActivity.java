            ...
            try {
                ...
                if (cursor.moveToFirst()) {
                    ...
                    cursor = db.query("USER",
                            ...
                            new String[]{Integer.toString(User.farm_id), User.username},
                            null, null, null);
                    if (cursor.moveToFirst()) {
                        String time_stamp = new Date().toString();
                        FarmAideDBHelper.insertTransaction(db, cursor.getInt(0), User.farm_id, time_stamp, "Added " + value_string + "kg supply to " + feed_name);
                    }
                    finish();
            ...
            try {
                ...
                if (cursor.moveToFirst()) {
                    if (cursor.getDouble(1) <= value)
                        ...
                        if (cursor.moveToFirst()) {
                            String time_stamp = new Date().toString();
                            FarmAideDBHelper.insertTransaction(db, cursor.getInt(0), User.farm_id, time_stamp, "Consumed " + value_string + "kg supply from " + feed_name);
                        }
                        finish();
                        startActivity(getIntent());
                    }
